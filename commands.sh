...
tzc originate contract potato-game-v5 transferring 0 from alice running \
    "$(pwd)/build/potato.tz" --init "Pair None {}" --burn-cap 1 \
...
tzc transfer 0 from alice to potato-wallet-alice-v1 --entrypoint "hotPotato" \
    --arg "Pair \"<potato-game-v5>%new_game\" (Pair 42 (Pair 0 3))" --burn-cap 1

tzc transfer 1.0 from bob to potato-game-v5 --entrypoint "buy_ticket_for_game" \
    --arg "\"<potato-wallet-bob-v1>%receive\"" --burn-cap 1

tzc transfer 0 from alice to potato-game-v5 --entrypoint "start_game" --burn-cap 1

tzc transfer 0 from bob to potato-wallet-bob-v1 --entrypoint "send" \
    --arg "Pair \"<potato-game-v5>%pass_potato\" 42" --burn-cap 1

tzc transfer 0 from alice to potato-game-v5 --entrypoint "end_game" --burn-cap 1
